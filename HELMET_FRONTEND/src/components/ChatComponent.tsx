...
export default function ChatComponent() {
	...
	useEffect(() => {
		...
		const fetchMessages = async () => {
			const response = await fetch(`http://backend-helmet-lb-1240358724.us-east-1.elb.amazonaws.com/messages/${userId}/conv_1`);
	...
	const handleSendMessage = () => {
		if (newMessage.trim()) {
			...
			fetch('http://backend-helmet-lb-1240358724.us-east-1.elb.amazonaws.com/messages', {
				method: 'POST',
				headers: {
		...
		<div className={`flex items-center space-x-3 p-3 rounded-2xl ${
			...
			<button
				...
				className={`p-2 rounded-full ${
					message.sender === 'me' ? 'bg-orange-200' : 'bg-gray-100'
				}`}
			>
			...
			<div className="flex-1">
				<div className={`h-1 rounded-full ${
					message.sender === 'me' ? 'bg-orange-200' : 'bg-gray-200'
				}`}>
					<div className={`h-full w-1/3 rounded-full ${
						message.sender === 'me' ? 'bg-orange-400' : 'bg-gray-400'
					}`}></div>
				</div>
			</div>
