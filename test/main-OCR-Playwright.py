...
class DreamSeeker(DreamSeekerAgentSetup):
    def __init__(self):
        ...
        Infos: Tu possèdes une mémoire des 2 chats précédents (question + réponse). 
        Si tu as déjà répondu à une question similaire, utilise cette mémoire pour éviter les répétitions inutiles, 
        par exemple si tu as cliqué sur un bouton mais ça na pas donné de résultat satisfaisant pour passer à 
        l'étape suivant, parcours la page pour trouver une solution.
        Tu dois toujours proposer une action, même si c'est juste d'attendre ou de faire défiler la page.
        """
        super().__init__(name="DreamSeeker", system_prompt=system_prompt)
    
    def analyze_page_elements(self, ocr_data, page_url, objective):
        """
...
class OCR:
    ...
    def parse_image_data(self, image_path='screenshot.png'):
        ...
        # Increment step counter (define self.step_count = 0 in __init__)
        self.step_count += 1
        filename = f"ocr/step_{self.step_count}.json"

        # Write OCR data to file
        with open(filename, "w", encoding="utf-8") as f:
            json.dump(self.ocr_data, f, ensure_ascii=False, indent=2)

        print(f"🔖 OCR data saved to {filename}")
...
async def main():
    async with async_playwright() as p:
        ...
        # Start navigation
        objective = "réserver un voyage au Maroc"
        ...
        while iteration < max_iterations:
            ...
            action_data = dream_seeker.analyze_page_elements(ocr_data, current_url, objective)
