...
def search_similar_users(
    ...
    if k is not None:
        if not isinstance(k, int) or k <= 0 or k > 15:
            return json.dumps(
                {"success": False, "error": "❌ k must be an integer between 1 and 15"}
            )
    else:
        k = 5
    ...
    if min_experience_years is not None:
        if (
            not isinstance(min_experience_years, (int, float))
    ...
    try:
        ...
        for result in results:
            ...
            if (
                ...
                and user_experience < min_experience_years
            ):
                continue
            ...
            if trade_category_filter is not None:
                if not any(
                    trade_category_filter.lower() in category.lower()
                    for category in user_trade_categories
                ):
                    continue
        ...
        avg_similarity = (
            sum(user["similarity_score"] for user in final_results) / len(final_results)
            if final_results
            else 0
        )
        avg_experience = (
            sum(user["experience_years"] for user in final_results) / len(final_results)
            if final_results
            else 0
        )
        ...
        return json.dumps(
            {"success": False, "error": f"❌ Error searching similar users: {str(e)}"}
        )
